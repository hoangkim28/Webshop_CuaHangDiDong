...
class clientController extends Controller
{
    ...
    public function getHomePage(){
        $dataAllProduct = product::where('qty_product','!=','0')->OrderBy('created_at','DESC')->paginate(8);
    ...
    public function getAllProductPaginate(){
        $dataAllProduct = product::where('qty_product','!=','0')->orderby('created_at','ASC')->paginate(16);
    ...
    public function getSaleProductPaginate(){
        $dataSaleProduct = product::where('promotion_price','>=', 5)->orderBy('promotion_price','ASC')->paginate(8);
    ...
    public function getDetailProduct($idProduct){
        $productDetail = product::find($idProduct);
        $productComment = comment::where('id_product',$idProduct)->get();
        $productImages = image::where('id_product',$idProduct)->get();
        $id = $idProduct;
        // Lắng nghe sự kiện >>> tăng view
        Event::fire('client.products.detailProduct',$productDetail);
        
        return view('client.products.detailProduct',compact('productDetail','productComment','id','productImages'));
    }

    // Post bình luận - - >>
    public function AddComment(Request $reqs,$id)
    {
        $comment = new comment();
        $comment->id_product = $id;
        $comment->email = $reqs->txtEmail;
        $comment->name = $reqs->txtName;
        $comment->content = $reqs->txtContent;
        $comment->save();
        return redirect()->route('detailProductPage', $id)->with('thongbao',"Bình luận thành công");        
        ...
        if ($req->session()->has('cart')) {
            $this->validate($req,
                [
                    ...
                    'txt_order_phone'=> 'required|max:11|min:9',
           ...
           if ($req->txt_order_payment == "COD"){
               ...
               $to_name = $req->txt_order_name;
               $to_email = $req->txt_order_email;
               $data= array('dataCustomer'=>$dataCustomer);
               Mail::send('client.email', $data, function ($message) use ($to_name, $to_email) {
            ...
            return redirect()->back()->with('thongbao',"Giỏ hàng trống");}
    }
...
/** =========== Begin: Trang tin tức ========================*/
    ...
    public function getBillStatus($id)
    {
        ...
        return view('admin.orderdetail', compact( 'dataBill','dataBillDetail','dataProduct', 'dataCustomer'));
    }
